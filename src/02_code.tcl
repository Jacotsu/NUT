...
# Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA.
	...
	set ::SQL_THREAD [thread::create " package require sqlite3 ;
		sqlite3 db $DiskDB;
		db timeout 10000 ;
	...
	if {$appSize != 0.0} {
		set ::ALTGUI 1
		db eval {select code from tcl_themes} {
			eval $code
 		}

		#db eval {select theme from options} {
		#puts $theme
...
dbmem eval {
	...
	SELECT    f.ndb_no,
						2008,
						ifnull(vite_added.nutr_val, 0.0) + ifnull(tocpha.nutr_val, 0.0)
	FROM      food_des f
	LEFT JOIN nut_data tocpha
	ON        f.ndb_no = tocpha.ndb_no
	AND       tocpha.nutr_no = 323
	LEFT JOIN nut_data vite_added
	ON        f.ndb_no = vite_added.ndb_no
	AND       vite_added.nutr_no = 573
	WHERE     tocpha.nutr_val IS NOT NULL
	OR        vite_added.nutr_val IS NOT NULL;
	...
	gain.
	theme     The user's selected theme
	*/
	CREATE TABLE IF NOT EXISTS options
							 (
														...
														theme TEXT DEFAULT "highcontrast"
							 );
	...
	*/
	CREATE TABLE IF NOT EXISTS archive_mealfoods
							 (
														...
														PRIMARY KEY(meal_id desc, ndb_no ASC, meals_per_day)
							 );
	...
	*/
	CREATE TABLE IF NOT EXISTS z_tu
							 (
														...
														ndb_no  INT,
														nutr_no INT,
														PRIMARY KEY(meal_name, ndb_no),
														UNIQUE(meal_name, nutr_no)
							 );
	...
	*/
	CREATE TABLE IF NOT EXISTS z_wl
							 (
														...
														wldate    INT,
														cleardate INT,
														PRIMARY KEY(wldate, cleardate)
							 );

	/* To protect table options from extraneous inserts we create a trigger */
	DROP TRIGGER
	IF EXISTS protect_options;CREATE TRIGGER protect_options after
			INSERT
			ON options begin
			DELETE
			FROM   options
	...
	INSERT INTO options default VALUES;DROP TRIGGER protect_options;vacuum;}
...
proc load_logic {args} {
	...
	*/ 
	...
	CREATE TRIGGER am_analysis_header_trigger after update of am_analysis_header on z_trig_ctl when NEW.am_analysis_header = 1 begin update z_trig_ctl set am_analysis_header = 0;
	...
	CREATE TRIGGER rm_analysis_header_trigger after update of rm_analysis_header on z_trig_ctl when NEW.rm_analysis_header = 1 begin update z_trig_ctl set rm_analysis_header = 0;
	...
	CREATE TRIGGER am_analysis_minus_currentmeal_trigger after update of am_analysis_minus_currentmeal on z_trig_ctl when NEW.am_analysis_minus_currentmeal = 1 begin update z_trig_ctl set am_analysis_minus_currentmeal = 0;
	delete from z_anal;
	...
	*/
	...
	CREATE TRIGGER am_analysis_null_trigger after update of am_analysis_null on z_trig_ctl when NEW.am_analysis_null = 1 begin update z_trig_ctl set am_analysis_null = 0;
	delete from z_anal;
	...
	CREATE TRIGGER rm_analysis_null_trigger after update of rm_analysis_null on z_trig_ctl when NEW.rm_analysis_null = 1 begin update z_trig_ctl set rm_analysis_null = 0;
	...
	*/
	...
	CREATE TRIGGER am_analysis_trigger after update of am_analysis on z_trig_ctl when NEW.am_analysis = 1 begin update z_trig_ctl set am_analysis = 0;
	...
	CREATE TRIGGER rm_analysis_trigger after update of rm_analysis on z_trig_ctl when NEW.rm_analysis = 1 begin update z_trig_ctl set rm_analysis = 0;
	delete from rm_analysis;
	...
	CREATE TRIGGER am_dv_trigger after update of am_dv on z_trig_ctl when NEW.am_dv = 1 begin update z_trig_ctl set am_dv = 0;
	delete from am_dv;
	...
	*/
	...
	create view z_pcf as select meal_id,NDB_No, Gm_Wgt + dv / meals_per_day * dvpct_offset / Nutr_Val * -1.0 as Gm_Wgt, Nutr_No from mealfoods natural join rm_dv natural join nut_data join options where abs(dvpct_offset) >= 0.05 order by abs(dvpct_offset);

	drop trigger if exists PCF_processing;
	CREATE TRIGGER PCF_processing after update of PCF_processing on z_trig_ctl when NEW.PCF_processing = 1 begin update z_trig_ctl set PCF_processing = 0;
	...
	*/
	...
	CREATE TRIGGER defanal_am_trigger after update of defanal_am on options begin update z_trig_ctl set am_analysis_header = 1;
	...
	*/ 
	...
	CREATE TRIGGER currentmeal_trigger after update of currentmeal on options begin update mealfoods set Nutr_No = null where Nutr_No is not null;
	...
	*/
	...
	CREATE TRIGGER z_n6_insert_trigger after insert on z_n6 begin update z_n6 set n6hufa = (select 100.0 / (1.0 + 0.0441 / p6 * (1.0 + p3 / 0.0555 + h3 / 0.005 + o / 5.0 + p6 / 0.175)) + 100.0 / (1.0 + 0.7 / h6 * (1.0 + h3 / 3.0))), reduce = 0, iter = 0;
	end;
		 /*
	...
	*/
	...
	CREATE TRIGGER z_n6_reduce6_trigger after update on z_n6 when NEW.n6hufa > OLD.FAPU1 and NEW.iter < 100 and NEW.reduce in (0, 6) begin update z_n6 set iter = iter + 1, reduce = 6, n6hufa = (select 100.0 / (1.0 + 0.0441 / (p6 - iter * .01 * p6) * (1.0 + p3 / 0.0555 + h3 / 0.005 + o / 5.0 + p6 / 0.175)) + 100.0 / (1.0 + 0.7 / (h6 - iter * .01 * h6) * (1.0 + h3 / 3.0)));
	end;
		 /*
		 Input:  If column "reduce" is set right, this trigger recursively
	...
	*/
	...
	CREATE TRIGGER z_n6_reduce3_trigger after update of n6hufa on z_n6 when NEW.n6hufa < OLD.FAPU1 and NEW.iter < 100 and NEW.reduce in (0, 3) begin update z_n6 set iter = iter + 1, reduce = 3, n6hufa = (select 100.0 / (1.0 + 0.0441 / p6 * (1.0 + (p3 - iter * .01 * p3) / 0.0555 + (h3 - iter * .01 * h3) / 0.005 + o / 5.0 + p6 / 0.175)) + 100.0 / (1.0 + 0.7 / h6 * (1.0 + (h3 - iter * .01 * h3) / 3.0)));
	end;
		 /*
	...
	*/
	...
	CREATE TRIGGER insert_mealfoods_trigger after insert on mealfoods when NEW.meal_id = (select currentmeal from options) and (select count(*) from mealfoods where meal_id = NEW.meal_id) = 1 begin update z_trig_ctl set am_analysis_header = 1;
	update z_trig_ctl set am_analysis_minus_currentmeal = case when (select mealcount from am_analysis_header) > 1 then 1 when (select mealcount from am_analysis_header) = 1 and (select lastmeal from am_analysis_header) != (select currentmeal from am_analysis_header) then 1 else 0 end;
	update z_trig_ctl set am_analysis_null = case when (select mealcount from am_analysis_header) > 1 then 0 when (select mealcount from am_analysis_header) = 1 and (select lastmeal from am_analysis_header) != (select currentmeal from am_analysis_header) then 0 else 1 end;
	update z_trig_ctl set rm_analysis_header = 1;
	update z_trig_ctl set rm_analysis = case when (select mealcount from rm_analysis_header) = 1 then 1 else 0 end;
	...
	*/
	...
	CREATE TRIGGER delete_mealfoods_trigger after delete on mealfoods when OLD.meal_id = (select currentmeal from options) and (select count(*) from mealfoods where meal_id = OLD.meal_id) = 0 begin update mealfoods set Nutr_No = null where Nutr_No is not null;
	update z_trig_ctl set am_analysis_header = 1;
	update z_trig_ctl set am_analysis_minus_currentmeal = case when (select mealcount from am_analysis_header) > 1 then 1 when (select mealcount from am_analysis_header) = 1 and (select lastmeal from am_analysis_header) != (select currentmeal from am_analysis_header) then 1 else 0 end;
	update z_trig_ctl set am_analysis_null = case when (select mealcount from am_analysis_header) > 1 then 0 when (select mealcount from am_analysis_header) = 1 and (select lastmeal from am_analysis_header) != (select currentmeal from am_analysis_header) then 0 else 1 end;
	update z_trig_ctl set rm_analysis_header = 1;
	...
	*/
	...
	CREATE TRIGGER update_mealfoods2weight_trigger AFTER UPDATE ON mealfoods when NEW.Gm_Wgt > 0.0 and (select block_setting_preferred_weight from z_trig_ctl) = 0 BEGIN update weight set Gm_Wgt = NEW.Gm_Wgt where NDB_No = NEW.NDB_No and Seq = (select min(Seq) from weight where NDB_No = NEW.NDB_No) ;
	end;
		 drop trigger if exists insert_mealfoods2weight_trigger;
	CREATE TRIGGER insert_mealfoods2weight_trigger AFTER INSERT ON mealfoods when NEW.Gm_Wgt > 0.0 and (select block_setting_preferred_weight from z_trig_ctl) = 0 BEGIN update weight set Gm_Wgt = NEW.Gm_Wgt where NDB_No = NEW.NDB_No and Seq = (select min(Seq) from weight where NDB_No = NEW.NDB_No) ;
	end;
				/*
	...
	*/
	...
	create view wlsummary as select case when (select weightn from z_wslope) > 1 then
...
set Start_NUT {
	...
	if {!$::ALTGUI} {
	 after 1000 {.nut.vf.frtreeview configure \
		-height [winfo height .nut.vf.nbw]}
	 after 1000 {.nut.vf.frtreeview configure \
		-width [winfo width .nut.vf.nbw]}
	 after 1000 {.nut.rm.frtreeview configure \
...
proc CancelSearch {args} {
	...
	if {!$::ALTGUI} {
	 ...
	 grid remove .nut.rm.frtreeview
	 grid .nut.rm.grams
	 grid .nut.rm.ounces
	 grid .nut.rm.analysismeal
	 } else {
	 place forget .nut.rm.searchcancel
	 place forget .nut.rm.frtreeview
	 place .nut.rm.grams \
		-relx 0.87 \
		-rely 0.0046296296 \
...
proc FindFoodrm {args} {
 ...
 after 350 FindFood_later {.nut.rm.frtreeview.treeview $::like_this_rm}
}
...
proc FindFoodvf {args} {
 ...
 after 350 FindFood_later {.nut.vf.frtreeview.treeview $like_this_vf}
}
...
proc FindFood_later {treeview regexFilter} {
	regsub \
	-all {
		"} "$regexFilter" " " regexResult
		regsub \
		-all "\{" "$regexResult" " " regexResult
			if { $regexFilter == ""} {
				$treeview delete [$treeview children {}]
		 	} else {
				set query "select Long_Desc from food_des where "
				foreach token $regexResult {
					append query "Long_Desc like \"%${token}%\" and "
				}
				append query "Long_Desc is not null order by Long_Desc collate
					nocase asc"

				$treeview delete [$treeview children {}]
				foreach {Long_Desc} [db eval $query] {
					$treeview insert {} end \
					-text "$Long_Desc"
				}
...
proc FoodChoicerm {args} {
 ...
 uplevel #0 {
  set ld "[.nut.rm.frtreeview.treeview item \
		[.nut.rm.frtreeview.treeview selection] -text]"
  if {$ld == ""} {return}
...
proc FoodChoicevf {args} {
 ...
 uplevel #0 {
  ...
  tuneinvf
  if {!$::ALTGUI} {
  grid remove .nut.vf.frtreeview
  grid .nut.vf.nbw
  grid .nut.vf.meal       } else {
  place forget .nut.vf.frtreeview
  place .nut.vf.nbw \
  -relx 0.0 \
  -rely 0.25 \
  -relheight 0.75 \
...
proc mealchange {args} {
		set ::mealoffset [expr {int($::mealoffset)}]
	 if {! $::realmealchange} {
...
proc drawClock {} {
    ...
    # Ziffernblatt
		...
		-fill black
		.loadframe.c create line 1 [expr {$::clockscale * 200 - 1}] [expr {$::clockscale * 200 - 1}] [expr {$::clockscale * 200 - 1}] [expr {$::clockscale * 200 - 1}] 1 \
		-fill "#8E8A8E"
		.loadframe.c create line 0 [expr {$::clockscale * 200}] 0 0 [expr {$::clockscale * 200}] 0 \
		-fill "#F8F8F8"
		.loadframe.c create line 1 [expr {$::clockscale * 200 - 2}] 1 1 [expr {$::clockscale * 200 - 2}] 1 \
		-fill "#D7D7D7"
	# Zeiger
...
proc initDB {args} {
	...
	db eval {create table if not exists z_tcl_code(name text primary key,
		code text)}
	db eval {create table if not exists tcl_themes(name text primary key,
		code text)}
	...
	db eval {BEGIN}
	db eval {insert or ignore into z_tcl_version values(null, $::version, NULL)}
	...
	db eval {insert or replace into z_tcl_code values('FindFood_later',$::FindFood_later)}
	...
	db eval {insert or replace into tcl_themes values('HighContrastTheme',$::HighContrastTheme)}
	db eval {COMMIT}

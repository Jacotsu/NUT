...
BEGIN
  ...
  FROM options LEFT JOIN
    (
      ...
      LIMIT
        (
          SELECT defanal_am FROM options
        )
    );
END;
  ...
  INSERT INTO z_anal
    SELECT
      ...
      (
        SELECT
          NDB_No,
          total(Gm_Wgt / 100.0 / mealcount * meals_per_day
      ) AS mhectograms
    FROM mealfoods
    JOIN am_analysis_header
    WHERE meal_id BETWEEN firstmeal AND lastmeal
      AND meal_id != currentmeal GROUP BY NDB_No)
    JOIN nutr_def NATURAL LEFT JOIN nut_data GROUP BY Nutr_No;
END;
...
BEGIN
  ...
  SET
    caloriebutton = (SELECT caloriebuttON FROM am_analysis_header),
    macropct = '0 / 0 / 0',
    n6balance = '0 / 0';
END;
...
AFTER UPDATE OF am_analysis ON z_trig_ctl
...
BEGIN
  ...
  UPDATE am_analysis_header
    SET
      macropct = (
        SELECT
          CAST(IFNULL(round(100 * PROT_KCAL.Nutr_Val
          / ENERC_KCAL.Nutr_Val,0),0) AS int) || ' / ' ||
          CAST(IFNULL(round(100 * CHO_KCAL.Nutr_Val / ENERC_KCAL.Nutr_Val,0),
          0) AS int) || ' / ' ||
          CAST(IFNULL(round(100 * FAT_KCAL.Nutr_Val / ENERC_KCAL.Nutr_Val,0),
          0) as int)
...
---------------------------[VIEWS TRIGGERS]----------------------------------
  ...
  UPDATE OF Gm_Wgt ON pref_Gm_Wgt
  WHEN NEW.Gm_Wgt > 0.0
  BEGIN
    UPDATE weight
...
BEGIN;
  ...
  SELECT NULL, NULL, NULL, 1, 1, 900.0 *
    CASE
      WHEN SHORT3.Nutr_Val > 0.0 THEN
        SHORT3.Nutr_Val
      ELSE
        0.000000001
    END /
    CASE
      WHEN ENERC_KCAL.Nutr_Val > 0.0 THEN
        ENERC_KCAL.Nutr_Val
      ELSE
        0.000000001
    END,
  ...
  BEGIN
    ...
    SET
      caloriebuttON = (
        SELECT caloriebuttON
        FROM am_analysis_header
      ),
      macropct = (
        SELECT
          cast(IFNULL(round(100 * PROT_KCAL.Nutr_Val / ENERC_KCAL.Nutr_Val,0),
          0) AS int)
          || ' / ' || cast (IFNULL(round(100 * CHO_KCAL.Nutr_Val /
          ENERC_KCAL.Nutr_Val,0),0) as int) || ' / ' ||
          cast (IFNULL(round(100 * FAT_KCAL.Nutr_Val / ENERC_KCAL.Nutr_Val,0),
          0) as int)
